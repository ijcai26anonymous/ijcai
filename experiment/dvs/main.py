    ...
    for idx, (ptns, labels) in enumerate(train_ldr):
        ...
        if model.step_mode == 's':
            ...
            for t in range(args.T):
                # For DVS data: [B, T, C, H, W], we take the t-th time step
                # For static data: [B, C, H, W], we take the whole image
                input_t = ptns[:, t] if len(ptns.shape) == 5 else ptns
                out = model(input_t)
            ...
            if len(ptns.shape) == 5:
                # For DVS data [B, T, C, H, W] -> [T, B, C, H, W] -> [T*B, C, H, W]
                in_data = ptns.transpose(0, 1).reshape(-1, *ptns.shape[2:])
            else:
                # For static data [B, C, H, W] -> [T*B, C, H, W]
                in_data = ptns.repeat(args.T, 1, 1, 1)
        ...
        with torch.no_grad():
            if len(ptns.shape) == 5:
                # Consistent with logit-snn: ANN teacher takes temporal mean
                t_avg_fr = t_model(ptns.mean(1))
            else:
...
def main():
    ...
    log.info('================================================================')
    log.info(
        'Finish training! Best accuracy: {:.4f} at epoch {}.'.format(best_acc, best_epoch))
