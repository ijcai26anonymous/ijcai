...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_layer4=False):
        out = self.conv0(x)
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out_layer4 = self.layer4(out)  # Save layer4 output
        out = self.avg_pool(out_layer4)

        out = out.reshape(-1, out.shape[1])
        out = self.classifier(out)
        
        T_actual = self.time_step if hasattr(self, 'time_step') else 1
        out = out.reshape(T_actual, -1, out.shape[1])

        if return_layer4:
            return out, out_layer4  # [T, B, num_classes], [T*B, 512, 4, 4]
        return out
...
class ResNet19(nn.Module):
    ...
    def forward(self, x, return_layer4=False):
        ...
        out = self.layer1(out)
        out = self.layer2(out)
        out_layer4 = self.layer3(out)  # Save layer3 output (ResNet19 only has 3 layers)
        out = self.avg_pool(out_layer4)
        ...
        if return_layer4:
            return out, out_layer4  # [T, B, num_classes], [T*B, 512, H, W]
        return out
